...
# (deterministic recoding — safe to do before split)
# ==========================================
...
for col in med_cols:
    df[col] = df[col].apply(simplify_med)
...
# (all deterministic row-wise math — safe before split)
# ==========================================
...
df["medication_complexity"] = (
    df["num_medications"].fillna(0) / (df["time_in_hospital"].fillna(0) + 1)
)
...
high_risk_discharge = [2, 3, 4, 5, 6, 10, 12, 14, 22, 23, 24, 25]
df["discharge_high_risk"] = (
    df["discharge_disposition_id"].isin(high_risk_discharge)
).astype(int)

df["previous_admission_intensity"] = (
    df["number_inpatient"].fillna(0) / (df["time_in_hospital"].fillna(0) + 1)
)


# ==========================================
# Age Engineering (fixed mapping — safe before split)
# ==========================================
...
# ==========================================
# A1C / Glucose Features (fixed mapping — safe before split)
# ==========================================

df["a1c_measured"] = (df["A1Cresult"].fillna("None") != "None").astype(int)
df["glu_measured"] = (df["max_glu_serum"].fillna("None") != "None").astype(int)
